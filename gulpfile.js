...
gulp.task('ship-to-gitlap', function (done) {
    ...
    for (var j = 0; j < cloneRepos.length; j++) {
        var cloneRepoName = cloneRepos[j] === 'common' ? 'common-aspcore' : cloneRepos[j] + '-asp';
        ...
        if (clone.code !== 0) {
            ...
            if (cloneRepos[j] === 'common') {
                ...
                var coreDestFiles = glob.sync(destPath + 'aspcore/*');
                for (var i = 0; i < coreDestFiles.length; i++) {
                    var copyPath = './src/' + cloneRepos[j] + coreDestFiles[i].split('src')[1].replace('/aspcore', '');
                    copyPath = copyPath + `${copyPath.indexOf('.md') === -1 ? '/*' : ''}`;
                    shelljs.cp('-rf', copyPath, coreDestFiles[i]);
                }
            }
            // end
